...
- 3）回溯查询：基于es实现文本回溯接口；基于astra中台的DHN特征提取、FAISS特征检索算法实现媒体数据的相似查询；
```
...
【项目难点】：
- 1）数据互通：外部门的异构词库数据的写入；业务高效接入；复杂度可维护；
- 2）运营优化：大词库的容量管理、节点的腾挪和调度；
- 3）生效优化：缩短种子从添加到服务生效的耗时；确保每一个种子都不会漏过或误杀；
【解决方案】：
- 1）种子互通：类网络协议的配置化和分层设计、支持字段和枚举值的映射；支持变量/常量/函数逻辑的取值模式；
- 2）运营优化：词库拆分；冷热分离；文本降噪/预处理（转换+过滤）；类k8s-hpa的弹性调度机制；
- 3）生效优化：定期全量+实时增量加载词、请求结果合并两个AC树；基于旁路delaychck服务实现种子生效的一致性告警；
```
